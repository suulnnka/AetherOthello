...
- `names`:挂上的开局名(可多个:序列目录的命名 + OCD 图案目录按局面挂名,
  分叉点上多开局共享一个局面属正常;**别名不进此字段**,2026-09-21 用户决策
  —— 主名+别名是假并列,会顶掉本可单名显示的局面),84 个局面有名、
  25 个并列、56 个不同名字。blob 侧再单一化:并列不展示、无名沿树继承
  (见下「Zig 侧嵌入」)。
...
- **编码**:每条 = 初始局面起的 BFS 最短路径(着法 token,PASS=64)
  ...
  (u16 数量 + 每串 u8 len + ASCII)。名字**单一化**(2026-09-21 用户决策):
  单名局面显名、并列局面(≥2 命名开局换位汇成,25 个)不展示、无名局面
  沿树继承最近单名祖先(穿过并列点不断链)—— 名字池里没有「 / 」并列串,
  888/914 条带名。
- **实测**:raw 7,077 B / gzip 3,937 B(名字段净增 ~1.3 KB gz;旧自对弈书
  3,602/2,609)。
