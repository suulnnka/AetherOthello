...
上限可调(2026-09 确认):**按需上调到刚好覆盖的整数档,硬顶 64 KB**。
...
| **合计** | | **29,981 / 35,840 B** | **当前档余量 ≈ 5.7 KB;到 64 KB 硬顶共 ≈ 34.7 KB** |
...
7. **闸门不放水**:默认保持 35 KB;哪一项真的需要更多(预计只有 ⑩ 书),
   在**该项目的提交里**把 `check-size.mjs` 黑白棋预算提到刚好覆盖的整数档
   (如 40 KB),硬顶 64 KB。不提前调成 64 —— 闸门的意义是防无意膨胀,
   一次放到顶等于没有闸门。
