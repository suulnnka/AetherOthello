...
/// 相位数 = 3:同一局部形状的价值随局势推进变化(开局行动力为王 → 残局稳定子
/// 为王),一套权重表达不了,所以按子数分档、每档一套完整的轨道权重。
/// 分界见 `phaseOf`(34/50)。档数必须在**训练时**定死:事后归并/拆分都有实测
/// 代价(kix4:6 档平均成 2 档差 5.2 子;2 档从零训对 6 档只差 1.7 子)。
/// 实测记录(各十万盘,A/B 400 局对 2 档书):6 档 +1.1±1.3、3 档 +0.2±1.3
/// —— 档数 2/3/6 之间无可分辨差异,定格 3 档取体积(权重书 28.4KB,一半)。
/// 两轮实验共同的真瓶颈:>34 区段 CG 停在热启动(条件数差),与档数无关。
...
///
/// ⚠ **version 3:每相位一个 scale,相位数由 blob[5] 声明**(头 12+4×phases,
///   3 相位 = 24 字节;v2 = 固定 2 相位 20 字节,v1 = 16 字节头共用 scale)。
///   每相位一个 scale 的理由:不同相位的权重幅值能差一倍,共用一个会让幅值
///   小的那个相位白丢一半分辨率。
...
/// 阶段划分:子数 ≤34 → 相位 0,≤50 → 相位 1,更高 → 相位 2。
/// · 34 是旧 2 相位时代的分界(v8 已验证),保留它 ⇒ v2 的书可以**无损展开**
///   成 3 档当训练初始(相位 0 ← 旧相位 0,相位 1/2 ← 旧相位 1);
/// · 50 把"6 档实验里 CG 滞住的 35~54 尾中段"拆成两半:35~50 独占相位 1
///   (16 个子数档的数据,参数条件数比 6 档时好得多),51~64 与之合并互相供血。
/// 阶段数必须在**训练时**定死:事后归并/拆分都有实测代价(见文件头)。
pub inline fn phaseOf(discs: u32) usize {
    if (discs <= 34) return 0;
    if (discs <= 50) return 1;
    return 2;
}
...
///
/// ⚠ 收**指针**而不是值:`[PHASES][ORBITS]f32` 是 113,700 字节(3 相位),按值传会在
