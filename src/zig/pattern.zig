...
//
// ── 二进制里只放什么 ──────────────────────────────────────────
//   只放 PHASES × 9,475 = 28,425 字节(3 相位)的 int8 权重。轨道图(每槽 →
//   轨道号)在 init() 时算出来(约 1,300 万次 comptime 已知的移位/查表,几十毫秒),
...
/// 相位数:同一局部形状的价值随局势推进变化(开局行动力为王 → 残局稳定子
...
/// 噪声内(P=3/5 的分界不含 34,基线被展开削弱,读数偏高);P3 与 P4 直接
/// 对打 200 局亦平手(-2.8±1.5,z<1)。**定格 3 档**:与 4 档等强,书小 9.5KB。
...
/// 公式是 kix4 的 1 基 ceil(f/SPAN) − 1(f = 子数 − 4,f=0 并入第 0 档);
/// P=2 时分界恰在 34,与旧 2 相位时代的切法重合,v2 书可无损展开。
/// 阶段数必须在**训练时**定死:事后归并/拆分都有实测代价。
pub inline fn phaseOf(discs: u32) usize {
    const f = discs -| 4; // 0..60
    return @min((f -| 1) / SPAN, PHASES - 1);
}
